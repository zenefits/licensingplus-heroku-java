...
public class Reconciler extends Thread {
	...
	public Map<String, LicenseInternal> ProcessInfoInSalesForce(Map<String, LicenseInternal> aInLicenses,
		...
		if (lFailedRequests.size() > 0) {
			// Send an email with the alert
			SendGridClient.sendEmailv2("License Sync Errors",
					"Failed to update " + lFailedRequests.size() + " licenses in sales force, check UI for details");
		}

		return lFailedRequests;
	}

	public void PostToSalesForce(List<LicenseInternal> aInLicenses, Map<String, LicenseInternal> aInOutFailedRequests,
			Map<String, LicenseInternal> aInAllRequests, boolean aInRetry, StringBuilder aInOutErrors) {

		String lMsg = "";
		System.out
				.println("Reconciler: Call to Post to sales force, size " + aInLicenses.size() + " retry " + aInRetry);
		try {
			...
			for (LicenseResponse lLicenseResponse : lLicenseResponses) {
				...
				if (lLicenseResponse.isSuccess()) {
					...
					System.out.println("Reconciler: License Key " + lKey + " ignore data error " + lLicenseResponse.getErrorCode());
				} else {
					...
					if (aInAllRequests.containsKey(lKey)) {
						...
						lLicense.lastErrorCode = (CalenderUtils.isNullOrWhiteSpace(lLicenseResponse.getErrorCode()))
								? "UNKNOWN" : lLicenseResponse.getErrorCode();
